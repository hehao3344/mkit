...
int packet_enc(char * buf_in, int len_in, char * buf_out, int * out_len)
{
    ...
    {
        ret = 0;
        memcpy(buf_out, &head, sizeof(PACKET_HEAD));
        memcpy(buf_out + sizeof(PACKET_HEAD), buf_in, len_in);
        if (padding_len > 0)
        {
            memset(buf_out + sizeof(PACKET_HEAD) + len_in, 0, padding_len);
        }
...
int packet_dec(char * buf_in, int len_in, char * buf_out, int * out_len)
{
    ...
    if (len_in > sizeof(PACKET_HEAD))
    {
        ...
        {
            ret = 0;
            memcpy(buf_out, buf_in + sizeof(PACKET_HEAD), head.valid_len);
